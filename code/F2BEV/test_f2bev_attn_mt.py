...
# -*- coding: utf-8 -*-
"""
Created on Thu Feb 23 14:05:34 2023
...
if not os.path.exists('./predictions/f2bev_attn_mt/'):
   os.makedirs('./predictions/f2bev_attn_mt/')
if not os.path.exists('./predictions/f2bev_attn_mt/bevfeatures'):
   os.makedirs('./predictions/f2bev_attn_mt/features')
if not os.path.exists('./predictions/f2bev_attn_mt/predfull/'):
   os.makedirs('./predictions/f2bev_attn_mt/predfull/')
if not os.path.exists('./predictions/f2bev_attn_mt/predfull/ce/'):
   os.makedirs('./predictions/f2bev_attn_mt/predfull/ce/')
...
checkpoint = torch.load('./f2bev_attn_mt.pt')
        ...
        for batch_idx, (dataseq, targetseq) in enumerate(test_dataloader):
            ...
            for ctr in range(seq_len):
                ...
                if batch_idx == 0:
                    ...
                    prev_bev = torch.Tensor([np.load('./predictions/f2bev_attn_mt/bevfeatures/'+image_list[batch_idx-1].split('.')[0]+'.npy')]).to(device)
                spred,_, hpred, _ ,for_prev_bev = model(front,left,rear,right, list(can_buses),prev_bev)

                for i,p in enumerate(for_prev_bev):
                    np.save('./predictions/f2bev_attn_mt/bevfeatures/'+image_list[batch_idx].split('.')[0]+'.npy',p.detach().cpu().numpy())
                for i,p in enumerate(hpred):
                    np.save('./predictions/f2bev_attn_mt/predfull/ce/'+image_list[batch_idx].split('.')[0]+'h.npy',p.detach().cpu().numpy())
                for i,p in enumerate(spred):
                    np.save('./predictions/f2bev_attn_mt/predfull/ce/'+image_list[batch_idx].split('.')[0]+'s.npy',p.detach().cpu().numpy())
