...
if not os.path.exists('./predictions/f2bev_conv_st_seg/bevfeatures'):
   os.makedirs('./predictions/f2bev_conv_st_seg/features')
...
checkpoint = torch.load('./f2bev_conv_st_seg.pt') 
    ...
    with torch.no_grad():
        for batch_idx, (dataseq, targetseq) in enumerate(test_dataloader):
            ...
            for ctr in range(seq_len):
                ...
                test_iou += jaccard_index(pred,target.to(pred.device),num_classes=5,average='none')
