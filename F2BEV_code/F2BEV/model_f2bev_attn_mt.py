...
# -*- coding: utf-8 -*-
"""
Created on Thu Feb 23 12:03:40 2023
...
from bblocks.mask_head_decoder_htseg import MaskHeadDecoderHt, MaskHeadDecoderSeg
...
class FisheyeBEVFormer(nn.Module):
    def __init__(self):
        ...
        self.segdecoder = MaskHeadDecoderSeg()
        self.htdecoder = MaskHeadDecoderHt()
    ...
    def forward(self,front,left,rear,right,can_buses,prev_bev=None):      
        ...
        seg_output, seg_inter_masks  = self.segdecoder(bevfeatures)
        ht_output, ht_inter_masks  = self.htdecoder(bevfeatures)


        if self.deep_supervision:
            return seg_output, seg_inter_masks, ht_output, ht_inter_masks, bevfeatures
        else:
            return seg_output, ht_output, bevfeatures
